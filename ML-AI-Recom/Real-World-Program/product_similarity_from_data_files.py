...
# When i show related products in the real application, i dont want to have to refactor the-
# matrix each time because its too slow.

# Instead, i can use the product features file to calculate product similarit quickly.

# Load prediction rules from data files
M = pickle.load(open("product_features.dat", "rb"))

# The M matrix that i just loaded has one column for each movie.
# I will transpose the the matrix so each column becomes a row. 
